...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        formatted = []
        sources = []  # Track sources for the UI as dicts
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            # Build context header (sent to Claude, unchanged)
            header = f"[{course_title}"
            if lesson_num is not None:
                header += f" - Lesson {lesson_num}"
            header += "]"

            # Deduplicate: same course + lesson should appear only once
            source_key = (course_title, lesson_num)
            if source_key not in seen_sources:
                seen_sources.add(source_key)

                # Build source label for UI
                label = course_title
                if lesson_num is not None:
...
class CourseOutlineTool(Tool):
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": "Get the complete outline of a course including course title, course link, and all lessons with their numbers and titles. Use this for questions about course structure, syllabus, outline, or lesson listings.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "course_name": {
                        "type": "string",
                        "description": "Course title or partial name to get the outline for (e.g. 'MCP', 'Introduction to Python')"
                    }
                },
    ...
    def execute(self, course_name: str) -> str:
        ...
        # Get course metadata by ID (title is ID)
        try:
            ...
            # Store source for UI
            self.last_sources = [{"label": course_title, "link": course_link}]
